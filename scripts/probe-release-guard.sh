...
if [ "$(api_json "$VW_API/pulls?state=open&per_page=100" "$prs_body")" != "200" ]; then
	# **ここは黙って続けてはいけない。** 一覧が取れないまま「open な PR は無い」と
	# 読むと、open な PR のプローブを落としたビルドを公開してしまう。
	die "open な PR の一覧を取得できませんでした（pull-requests: read がありますか）: $(api_message "$prs_body")"
fi
PRS_LIST="$(jq -r '[.[].number] | sort | map(tostring) | join(",")' "$prs_body" 2>/dev/null || true)"
