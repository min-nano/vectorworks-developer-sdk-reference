...
reset() {
	...
	: >"$LIST_CALLS_FILE"
	# **60 のすぐ上に置かない。** wait_run_or_successor は乗り換える前に
	# 「残り時間が 60 秒を切っていたら待ち直さない」と決めており、残り時間は
	# `date +%s` の**秒**で測る。TIMEOUT=60 だと、開始からこの判定までに秒が
	# 1 つ繰り上がっただけで残り 59 秒となり、**乗り換えずに打ち切る**——
	# 試験が秒の境目をまたぐかどうかで通ったり落ちたりする（実際に落ちた:
	# 「いちばん新しい後続を待つ」が run 105 ではなく 100 を返した）。
	# 打ち切りそのものは「残り時間が無いとき」が TIMEOUT を自分で下げて試すので、
	# ここは**余裕のある値**にしておく。
	TIMEOUT=600
	MAX_HANDOFFS=4
}
