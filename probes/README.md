...
確かめるための **1 ファイル完結のスニペット**を置く。

> **実際に走らせて確かめたいなら [`runtime/`](runtime/README.md)。** ここのスニペットは
> 構文チェック（`-fsyntax-only`）までで、実行はしない。「呼んだら何が起きるか」は
...
なお `compile` モードのインクルードパスには `plugin/src` も入っているので、
実機プローブ（`probes/runtime/<slug>/probe.cpp`。`#include "Probe.h"` する）も
同じモードで構文チェックできる。
