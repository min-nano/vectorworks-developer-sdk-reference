...
//
//	## ここまでの実測（VW 2026 / mac。PR #69 のコメント 3 本）
//
//	**1 回目・2 回目で確定したこと**:
//
//	  * **⓪ 曲線も ① `CreateCustomObjectPath` も、渡した値をビット一致で保つ**
//	    （2 回目は 40 ケース全件で「①の誤差 = 0」）。`NoOffset` でも同じ。
//	    **つまり生成の段では単位変換されない。**
//	  * **残差は ② 作り直し（regen）で生まれる。** バウンドを 1 本も書かずに
//	    `ResetObject` して 0 長へ潰すとき、値によっては厳密な 0 にならず
//	    **渡した長さの 1〜2 ULP** が残る（符号は長さの向きと同じ）。
//	  * **事故の残差はこれである。** `572 → 3531` は 3 回の実行・4 つの入口
//	    （断面あり／なし／NoOffset／走査）で毎回 `+4.5474735088646412e-13`
//	    ——事故で潰れた 46 本の値とビット一致。
//	  * **退化（z0 = z1）は 4 値とも厳密に 0。** プラグインの直し方は値に依らず効く。
//
//	**2 回目で落ちた仮説**: `v / 25.4 * 25.4 != v`（mm↔インチの往復）で危険な値を
//	予測できる、という issue #67 の仮説は**当たらない**。18 値の走査を含む 36 ケースで
//	一致 23 / 不一致 13——「予測あり・残差なし」が 12 件（1・2・13・103・813・1626・6503 …）、
//	**「予測なし・残差あり」が 1 件（572 → 3）**。SDK の逆数形（`v * (1/25.4) * 25.4`）は
//	さらに悪い（一致 17 / 不一致 19）。1 回目に 8/8 で当たって見えたのは、**仮説に
//	寄せた値ばかりを選んでいた**ため。**単位の往復では説明が付かない。**
//
//	残差が出た 5 件（z0 → z1 ／ 長さ ／ 残差）:
//
//	    572 → 3531 ／ 2959  ／ +2^-41（長さの 1 ULP）
//	     13 → 3531 ／ 3518  ／ +2^-40（2 ULP）
//	    572 → 8002 ／ 7430  ／ +2^-39（2 ULP）
//	    572 → 3    ／ -569  ／ -2^-43（1 ULP）
//	    572 → 4000 ／ 3428  ／ +2^-40（2 ULP）
//
...
//	1 回目のログの「予測B」列はこの縮約された値だった。いまは `volatile` で止めてある。
//
//	## この版（3 回目）で決めること
//
//	残差が**何で決まるか**を、3 つの群で挟み撃ちにする。
//
//	  * **H 群: 同じ長さ・違う端点。** 長さ 2959 を (0→2959) (100→3059) (572→3531)
//	    (1000→3959) (3531→6490) の 5 通りで渡す。**残差が全部同じなら「長さで決まる」**、
//	    572→3531 だけなら「端点で決まる」。
//	  * **I 群: 同じ上端・違う下端。** z1 = 4000 を 5 つの下端から。
//	  * **J 群: 繰り返し。** 残差が出た値・出なかった値を**順番を変えて 3 巡**通す。
//	    **同じ値がいつも同じ残差を出すのか**（＝値で決まるのか、その場の状態で変わるのか）。
//
		 ...
		 "新規の空図面で走る")
{
	...
	// =======================================================================
	probe.log("=== H. **同じ長さ・違う端点**（残差は長さで決まるのか、端点で決まるのか） ===");
	{
		// 長さはすべて 2959（572 → 3531 と同じ）。**残差が全部同じなら長さ、
		// 572→3531 だけなら端点**（どちらでもない並びなら、その値をそのまま持ち帰る）。
		const double bases[] = {0, 100, 572, 1000, 3531};
		for (size_t i = 0; i < sizeof(bases) / sizeof(bases[0]); ++i)
			runCase("H-長さ2959-下端" + Num(bases[i]), 0, bases[i], 0, bases[i] + 2959, true, false,
					false, "");
	}

	// =======================================================================
	probe.log("=== I. **同じ上端・違う下端**（z1=4000。2 回目に残差が出た組を崩す） ===");
	{
		const double bases[] = {0, 100, 572, 1000, 2000};
		for (size_t i = 0; i < sizeof(bases) / sizeof(bases[0]); ++i)
			runCase("I-上端4000-下端" + Num(bases[i]), 0, bases[i], 0, 4000, true, false, false,
					"");
	}

	// =======================================================================
	probe.log("=== J. **繰り返し**（同じ値がいつも同じ残差を出すか。順番を変えて 3 巡） ===");
	{
		// 2 回目に残差が出た 3 値（3531 / 4000 / 3）と、出なかった 2 値（6374 / 1626）。
		// **値で決まるならこの 15 行は 3 巡とも同じ**になる。順番を変えてあるので、
		// 「その場の状態（何番目に作ったか）」で決まっているならそこで崩れる。
		const double order1[] = {3531, 4000, 3, 6374, 1626};
		const double order2[] = {1626, 6374, 3, 4000, 3531};
		const double order3[] = {3, 3531, 1626, 4000, 6374};
		const double* orders[] = {order1, order2, order3};
		for (int round = 0; round < 3; ++round)
			for (size_t i = 0; i < 5; ++i)
				runCase("J" + std::to_string(round + 1) + "-" + Num(orders[round][i]), 0, 572, 0,
						orders[round][i], true, false, false, "");
	}
