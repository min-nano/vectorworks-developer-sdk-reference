...
//
//	実機の事故（[あちらの PR #113](https://github.com/min-nano/vectorworks-plugin-import-ifc-homeskz/pull/113)）
//	では、同じ取り込みの中で柱 197 本のうち 46 本だけが長さ 0 で描かれた。3 地点の
//	読み戻しで分かっているのは次の形である（issue #61 の表）:
//
//	    │ ① Create 直後 │ ② バウンド 2 本を書いた直後 │ ③ ResetObject 直後
...
//	## 測り方の方針
//
//	* **長さ以外はすべて同じにする。** 階・レベル種別・レイヤ・バウンド（#59 の A の形＝
//	  `{Story,自階,L,0}` / `{Story,上階,L,0}`）を固定し、`CreateCustomObjectPath` へ渡す
//	  2 点目の Z だけを振る。1 ケース 1 オブジェクト（前のケースの残りを持ち込まない）。
...
namespace
{
	...
	void LogPath(vwprobe::Report& probe, const std::string& prefix, MCObjectHandle pio)
	{
		...
		probe.log(prefix + " NurbsGetNumPts=" + std::to_string(static_cast<long long>(pointCount)));
...
VW_PROBE("reset-object-path-rebuild-threshold", "ResetObject がパスを作り直す／温存する境目を測る",
		 ...
		 "4 ULP を含む）、同じ 2 本の Story バウンドを書いて ResetObject したときに、"
		 "パスが作り直されるか温存されるかを読み比べる")
{
	// =======================================================================
	...
	// 「共有するレベル種別がいちばん多い隣接ペア」で測る（#59 と同じ選び方。決め打ちの
	// 高さを持ち込まないので、事故のモデルでも別のモデルでも同じように読める）。
	StoryInfo lower;
	StoryInfo upper;
	TXString levelType;
	for (size_t i = 0; i < stories.size(); ++i)
		...
		for (size_t j = 0; j < stories.size(); ++j)
		{
			...
			for (size_t k = 0; k < allLevelTypes.size(); ++k)
			{
				...
				if (gSDK->GetLayerForStory(stories[j].handle, allLevelTypes[k]) == nullptr)
					continue;
				shared.push_back(allLevelTypes[k]);
			}
			...
			if (!shared.empty() && lower.handle == nullptr)
			{
				lower = stories[i];
				upper = stories[j];
				levelType = shared[0];
		...
		container = lower.anyLayer;
	{
		...
		probe.log("P. 使う階: 下=\"" + Str(lower.name) + "\" 上=\"" + Str(upper.name) +
				  "\" / レベル種別 L=\"" + Str(levelType) + "\" / 置くレイヤ=\"" + Str(layerName) +
				  "\"");
	}
	gSDK->SetCurrentLayer(container);

	// バウンドは #59 の A の形（＝実務で普通の形。両方 Story で階だけが違う）で固定する。
	...
	// **オブジェクトを作らずに解ける値**（物差し）。作り直しが起きればパスの長さはこの差に
	// なるはずで、まとめの表の判定もこれと突き合わせる。
	const WorldCoord pureLowerZ = gSDK->GetStoryObjectDataBoundHeight(boundLower, container);
	const WorldCoord pureUpperZ = gSDK->GetStoryObjectDataBoundHeight(boundUpper, container);
	const double expectedHeight = pureUpperZ - pureLowerZ;
	probe.log("P. GetStoryObjectDataBoundHeight: {Story,0,L,0}=" + Num(pureLowerZ) +
			  " / {Story,1,L,0}=" + Num(pureUpperZ) +
			  " → 作り直されたときの長さ=" + Num(expectedHeight));
	if (expectedHeight == 0)
	{
		probe.fail("P. 2 本のバウンドが同じ絶対Zへ解決される（差が 0）。この図面では"
				   "「作り直された」と「潰れた」が見分けられないので、別のレベル種別を持つ"
				   "モデルで走らせ直してほしい。");
		return;
	}
					   ...
					   const std::string& why)
	{
		...
		// 判定は「作り直されたか」の 1 点。長さが解決済みバウンドの差（符号はどちらでも
		// よい。#59 で ID0=始点 / ID1=終点と確定しているが、符号は作り方に依る）に
		// なっていれば作り直された。
	...
	// =======================================================================
	probe.log("=== A. 長さだけを振る（基準Z＝下端の解決Z。ここがこのプローブの本体） ===");
	{
		const double baseZ = pureLowerZ;
		struct Step
		{
			const char* label;
			int ulps;		 // 非 0 ならこの ULP 数で作る（絶対値は無視）
		...
		const Step steps[] = {
			...
			{"A-09 Z の差 = 1", 0, 1,
			 "#56 の 5 が作り直されたと報告している長さ（別のバウンドの形で。ここで再確認する）"},
		...
		for (size_t i = 0; i < sizeof(steps) / sizeof(steps[0]); ++i)
		{
			...
			const double deltaZ =
				step.ulps != 0 ? UlpsAway(baseZ, step.ulps) - baseZ : step.absolute;
			runCase(step.label, baseZ, 0, deltaZ, step.why);
		}
	}
	...
	{
		const double baseZ = pureLowerZ * 1000 + 1e6; // 図面から遠い、桁の大きい座標
	...
	{
		const double baseZ = pureLowerZ;
		runCase("D-0 dx=1000 / Z の差=1 ULP", baseZ, 1000, UlpsAway(baseZ, 1) - baseZ,
				"判定が「曲線の長さ」なら作り直される。「Z の差」なら A-01 と同じ結果になる");
		runCase("D-1 dx=1000 / Z の差=0", baseZ, 1000, 0,
	...
	// 差し替えてもう一度 ResetObject する」形で見る。
	{
		...
		for (size_t i = 0; i < sizeof(replacements) / sizeof(replacements[0]); ++i)
		{
			...
			MCObjectHandle path = MakeStraightPath(WorldPt3(0, 0, pureLowerZ),
												   WorldPt3(0, 0, pureLowerZ + expectedHeight));
			MCObjectHandle pio =
			...
			// 局所座標で作る（挿入点は動かさない）。ULP 指定のときは、事故と同じ
			// 「ビットで隣」を**挿入点の高さのところ**で作ってから差へ落とす。
			const double deltaZ = replacement.ulps != 0
									  ? UlpsAway(pureLowerZ, replacement.ulps) - pureLowerZ
	...
	probe.log("作り直されたときの長さ = " + Num(expectedHeight) +
			  "（＝上階の解決Z − 自階の解決Z）");
