...
- **役目を終えたら消す。** 結論は `Findings/` に文章で残る（CLAUDE.md「調査のフロー」4）。
  **常駐の煙試験は置かない**——ピッカーに「調べているわけではないプローブ」が並ぶと、
  ...
  通っているかは、そのとき調べているプローブを走らせれば分かる。
...
# ③ 実機で走らせてもらう（結果は自動で PR へ返る）
利用者がすることは「入れ替える」と「走らせる」の 2 つだけ。走らせ終えると、**その
プローブが来た PR へ結果（結末・所要・出所・ログ全文）がコメントとして投稿される**
（plugin/README.md「結果を PR へ自動で返す」）。目印 `<!-- vw-probes-result v1 … -->`
で始まるコメントがそれで、**人は 1 文字も書いていない**（絵や所見はチャットへ来る）。
