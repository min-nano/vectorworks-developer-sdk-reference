...
| | |
| --- | --- |
| **宛先を尋ねない** | 投稿先の PR は**プローブの出所**が知っている（ビルドのときに決まる。`scripts/gather-probes.sh`）。番号を人に打たせない。**main のプローブは投稿しない**——もう Findings になった調査なので、宛先が無い |
...
| `scripts/vw-probes-feedback.{sh,ps1}` | 網とトークンの保管（**C++ は curl もトークンも触らない**）。`tests/vw-probes-feedback.test.sh` |
