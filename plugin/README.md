...
| 変わったもの | 落とすもの | 締め |
| --- | --- | --- |
| プローブだけ（ふつうはこちら） | `VwSdkProbesPayload-<mac\|win>.zip` | **何も尋ねない。** 次にメニューを開いたときから新しいプローブが動く |
| 殻まで | `VwSdkProbes.*.zip`（まるごと） | 「再起動しますか？」——押せば Vectorworks が終了して自動で起動し直す（開いている書類は保存を確認する） |
