...
//	Feedback.h
//
//	**プローブの結果を、その出所（PR、無ければ issue）へ自動で投稿する。**
//
...
//
//	【宛先を尋ねない】投稿先は**プローブの出所**が知っている（ビルドのときに決まり、
//	カタログと出所表に載っている。scripts/gather-probes.sh）。だから人に番号を打たせる
//	必要が無い。優先順位は 2 段:
//	  1. **PR。** 出所に無ければ、動いているビルドのブランチから open な PR を引く
//	     （下記「宛先の決め方」）。
//	  2. **issue。** PR が無いとき（main に入った後など）は、プローブ本体の先頭コメント
//	     にある `[issue #N]` へ投稿する。**閉じた issue へは投稿しない**（読まれない）
//	     ので、開いているかを同梱スクリプトの `issue-state` で確かめてから使う。
//	どちらも無ければ投稿しない（それでも Findings になった調査、または issue 番号を
//	書き忘れた調査で、実害は無い）。
//
...
namespace vwprobe
{
	...
	//   * pr が空で branch からも引けず、issue も無いか閉じている … false（宛先が無い）
	//   * 断られている                                            … false（尋ねない）
	//   * まだ尋ねていない                                        … ここで 1 度だけ尋ねる
	//   * トークンが無い                                          … ここで 1 度だけ貼り付けてもらう
	// pr は**入力かつ出力**——空のまま渡すと、branch から引けた番号がここへ入る
	// （上記「宛先の決め方」1）。issue も**入力かつ出力**——pr が最終的に空のままなら、
	// 開いていることを確かめたうえでそのまま使う。閉じていれば空にする（宛先が無い扱い）。
	// branch は動いているビルドのブランチ。note には利用者へ伝えたいことが入る
	// （呼び出し側が結果ダイアログへ添える）。
