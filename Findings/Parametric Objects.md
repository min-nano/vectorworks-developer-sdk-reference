...
## 構造材 PIO に「部材長」「スパン」を読めるパラメータは無い——実体はパスで測る

[issue #95](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/95)。
**実機確認済み**（VW 2026 / mac / 日本語環境。ホームズ君 IFC 取り込みプラグインの実機
フィードバック round 1 で、横架材 266 本・垂木 54 本・柱 197 本の全数）。

**描き上がった構造材がどれだけの実体を持っているか（長さ 0 で潰れていないか）を、
パラメータから読むことはできない。**

| 引いた名前 | 実機 |
| --- | --- |
| universal `Span` ／ ローカライズ「スパン」 | **解決しない**（`GetParamName` / `GetParamLocalizedName` の全走査で 1 件も一致しない） |
| `CenterPointLength`（ローカライズ「長さ」） | 引けるが**部材長ではない**——実長 5333mm の通し柱で **100** を返す |
| `StartElevation` / `EndElevation` | **解決済みの絶対 Z**。鉛直材はこの差が実体（上記「高さ・実体を最終的に決めるのは…」） |

「長さ」を名前に含むパラメータを全数列挙しても `CenterPointLength` の 1 件しか出ない
（実機のダンプそのまま。柱 1 本目）:

````
MajorDepth(主高さ)=120, MinorDepth(副高さ)=100, UseLayerCutPlaneElev(レイヤの切断面高さを使用)=0,
CutPlaneElev(切断面の高さ)=1000, DialogStartElevationReference(始端高さ基準)=0/"レイヤの高さ",
DialogStartElevation(始端高さオフセット)=0, DialogEndElevationReference(終端高さ基準)=0/"レイヤの高さ",
DialogEndElevation(終端高さオフセット)=0, StartElevation(始端高さオフセット)=572,
EndElevation(終端高さオフセット)=5905, CenterPointLength(長さ)=100
・図面のパス[piece0:2点 (0, 0, 0) (0, 0, 5333) / piece1:0点]
````

> **OIP に「スパン」の欄が見えていても、その名前でパラメータを引けるとは限らない。**
> 上記「パスの型を間違えると…」には OIP の表示として「スパン 0 / 長さ 0」の記録がある
> ——欄は見えている。**パラメータ表に「スパン」を含む名前が 1 件も無い**ことだけが
> 確かめられており、欄の出どころがパラメータでない（計算値）のか、まったく別の綴りで
> 持っているのかは**【未確認】**（issue #95 で追う）。

**実体を測るならパスを読む。** `GetCustomObjectPath` → `NurbsGetNumPts` → `NurbsGetPt3D` で
両端の距離を取る。`ResetObject` は解決済みストーリバウンドからパスを作り直し（始点が
ID 0 の解決 Z・終点が ID 1 の解決 Z）、**水平成分は渡したパスのままビット一致で残す**ので、
**リセット後のこのパスが「描かれた実体」そのもの**である（上記「高さ・実体を最終的に
決めるのは…」／「鉛直でないパスでは…」／「水平成分が 1e-7 以上ある部材は…」）。
**部材の向きを問わず同じ読み方で測れる**のが、パラメータには無い利点である。

- **読むのは必ず作り直しの後**（上記「検算できる。ただし読むのは『作り直しの後』」）。
- **鉛直材は `StartElevation` / `EndElevation` の差でも測れる**が、**水平材は両端の Z が
  等しいのが正常**なのでその差では全数が 0 になる。**測り方を向きで取り違えると、正常な材を
  「実体が無い」と誤報する**——そして「無い名前を引き続ける」ほうの取り違えは、**何も報せない
  まま検査が空振りする**という形で出る（横架材 266 本・垂木 54 本が正しく描けているのに、
  潰れ検出も自己修復も一度も動いていなかった）。**どちらの誤りも、実機の絵を見ただけでは
  気付けない。**
- 実装例: ホームズ君プラグインの `draw/DrawUtil` の `PioPathChord` と
  `draw/StructuralMember` の `MeasureDrawnMember`（`StructuralExtentKind`）。
