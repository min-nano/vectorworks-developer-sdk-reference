...
**ただし「0 長へ潰す」経路については、非鉛直のパスでも実機で確かめてある**——
**作り直しが触るのは Z の差だけで、X / Y の差はビット一致で残る**（下記「鉛直でないパスでは…」。
...
- **【未確認】潰れた斜め材は「0 長の死角」には落ちないかもしれない。** 潰れた後のパスは
  `(dx, dy, ±k ULP)` なので**3 次元長はほぼ `|水平成分|`** になり、上記の閾値
  （3 次元長が 1e-7 以上なら作り直す。#61 で実機確認済み）と合わせると
  「水平成分が 1e-7 以上ある斜め材は必ず作り直される＝『柱 46 本』の事故は鉛直材に固有」
  と読める。**ただしこの走査では、バウンドを持たせた 2 回目の `ResetObject` を測っていない。**
  確かめる手順ごと
  [issue #75](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/75) へ
  切り出した。
