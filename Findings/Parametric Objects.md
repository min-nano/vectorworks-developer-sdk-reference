...
バウンドの扱い）でもなく、**残っていたのはこれ**である。**ビット一致した個体（0 長）は
作り直され、1 ULP だけ残った個体（`4.54747e-13`）は作り直されなかった**。197 本中 46 本と
いう内訳は、その残差の当たり外れそのもの。実機で採った 3 地点の読み戻し
（`4.54747e-13` が ①②③ とも変わらない）は、上の表の 2 行目と同じ現象である。

> **【未確認】残差がどこで生まれたのかは、まだ分かっていない。**
> 当初ここには「事故の取り込みは両端の世界座標Zを**別々に計算しており**」と書いていたが、
> **プラグイン側で測ると合わない**——事故と同じ経路に渡っていた 2 つの world Z は、
> `スキップフロア_サンプル.ifc`（柱 197 本）で**非整数が 1 本も無く**、両端とも厳密な
> 整数だった（他の 2 フィクスチャでも 141 本中 1 本・138 本中 1 本）。**呼び出し側が
> 厳密でも `(0, 1e-7)` の帯に落ちうる**ということなので、「自分の計算を正せば避けられる」
> と読まないこと。
> 出どころの候補として **mm↔インチの往復**（`v / 25.4 * 25.4`）がある——潰れた柱の上端
> `3531` だけが往復で戻らず、その誤差 `-4.547473508864641e-13` は観測値と**ビット一致**
> する（無事だった `572` / `2429` / `6374` / `5905` はすべて誤差 0）。ただし整数 mm の
> 12.6% が戻らないので、**5 点の一致は偶然でも約 7% 起こる**。断定できる水準ではない。
> 切り出し先は
> [issue #67](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/67)。

**実務上の指針**:

- **両端の Z は同じ値（同じ変数）で渡し、厳密に退化させる。** 「ほぼ同じ」では駄目で、
  **1 ULP 残るだけで再構築が止まる**。**同じ値を渡せば、内部で何が起きても
  `f(z) − f(z)` は厳密に 0 になる**ので、残差の出どころ（上記【未確認】）が分からなくても
  これで避けられる——**別々の式で計算した結果を突き合わせない。**
