...
**7. ここまでは鉛直（`dx = dy = 0`）のパスの話である。** 斜め・水平では**別の規則**になる:

| 渡したパス | 潰した後 |
| --- | --- |
| `(0,0,0) → (2959, 0, 0)`（**水平**） | **潰れない**。X の差 2959 がそのまま残る（Z の差は元から 0） |
| `(0,0,0) → (2959, 0, 2959)`（**斜め**） | X の差 2959 はそのまま。**Z の残差は 0**（鉛直で同じ長さを渡すと 1 ULP なのに） |
| `(0,0,0) → (3000, 0, 3000)` | X はそのまま。**Z の残差は `−4.5474735088646412e-13`（−1 ULP）——符号が L と逆** |
| `(0,0,0) → (3428, 0, 3428)` | X はそのまま。Z の残差は `+9.0949470177292824e-13`（鉛直と同じ 2 ULP） |

- **潰れるのは Z の差だけで、X / Y の差は保たれる。**
- **X に値が入ると Z の残差が変わる**（2959 は 1 ULP → 0、3000 は 0 → **−1 ULP**）。
  つまり**「L だけで決まる」は鉛直パスに限った話**で、見ているのは 3 次元のパス全体である
  （上の「見ているのは Z の差ではなく 3 次元の長さ」と揃う）。
- **符号が L と逆になりうる。** 鉛直の 491 通りでは 1 件も出なかった。

非鉛直のときの規則は**この調査の範囲外**なので
[issue #73](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/73) へ
切り出した。**構造材 PIO を鉛直に使う限り、上の 1〜6 で足りる。**
