...
> **帰属についての訂正**（[issue #67](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/67)）:
> ここには当初「事故の取り込みは**両端の世界座標Zを別々に計算しており**、片方に丸めが
> 残った」＝**呼び出し側の算術**が原因だと書いていた。**これは誤りである。** 呼び出し側は
...
- **両端の Z は同じ値（同じ変数）で渡し、厳密に退化させる。** 「ほぼ同じ」では駄目で、
  **1 ULP 残るだけで再構築が止まる**。**長さ 0 で渡せば、値に依らず残差は出ない**
  （下記 #67 の B 群）——**これが唯一、値を選ばずに済む渡し方**である。
...
- 【推定】したがってこの部材は、**階の間隔が変われば長さも変わる**（ID 0 は自階基準、
  ID 1 は上階基準のまま、それぞれ固定の `fOffset` で解決されるため）。0 長へ差し替えた
  部材は「0 長で固まった」のではなく、**その瞬間の階間隔と釣り合う `fOffset` を持たされた**
  状態である。これは #59 で確定したバウンドの解決規則からの帰結で、**階を実際に動かして
  確かめてはいない**——
  [issue #65](https://github.com/min-nano/vectorworks-developer-sdk-reference/issues/65) へ
  切り出した。
- **差し替えを対症療法に使わない。** 潰れた部材を `SetCustomObjectPath` で直すと、
  図はその場で直るが**バウンドには嘘の `fOffset` が残る**。作るときに退化させて渡し、
  `ResetObject` に作り直させること（上記「実務上の指針」）。
