...
- **どの長さが残差を残すのか、規則は分かっていない。** 残差はどれも L の 1〜2 ULP
  （相対 1.5〜2.7e-16）で、**値そのもので決まり再現もする**が、mm ↔ インチの往復（両方の
  形）でも、1 に近い定数倍・二段の定数倍でも、上の表を再現する式は見つからなかった。
  **「危ない値の一覧」を式で作ることはできない**ので、実務では上記の指針
  （L = 0 に退化させる／作り直しの後に長さを読む）で避ける。規則そのものの追跡は
