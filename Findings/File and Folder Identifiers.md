...
**ヘッダ側の道は潰してある。** Windows 版 SDK と macOS 版 SDK は**配られるファイルとして
同一**なので（上記「`SAttributes` の中身（ヘッダ）」）、**`sdk-grep` / `sdk-ls` を
`--platform windows` で引き直しても答えは出ない——引き直さないこと。** 残っているのは
**Windows 実機でプローブを 1 つ走らせる 1 手だけ**である。
