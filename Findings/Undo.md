...
- **「コンパイルに成功しました」というダイアログを出しているのは誰か。** 利用者はこれを
  見ている。5 度目の走行で `CompileScript(showDialogs=false)` の所要が **1112ms**（他の
  呼び出しは 0〜17ms）だったので**この呼び出しが疑わしい**が、**これは走行中で最初の
  VectorScript エンジン呼び出しでもある**——初回の初期化に 1 秒掛かっただけ、という
  読み方も消えていない。**同じ `CompileScript` を続けて 3 回呼んで所要を並べれば分かる**
  （2 回目以降も 1 秒級ならダイアログ、0ms 級なら初期化）。6 度目のプローブで測る。
  - 分かれば「`showDialogs=false` は成功のダイアログを抑えない」という知見になる
    （抑えるなら、出所は `ExecuteScript` 側ということになる）。
- **VectorScript 版の実行時エラーの内容は、呼び出し側からは取れない**【ヘッダ根拠】。
  ...
  ホストへ通知する**逆方向の口。スクリプトから C++ へ値を返す近道も無い
  （`Repository_GetValue` はエンジン側の API で、スクリプトから書き込む `vs.*` の口は
  `vs.py` を検索して 0 件）。**画面には出るのに（上記のダイアログ）、呼び出し側には
  渡ってこない。** VectorScript 側の足跡は、**スクリプト自身に図形を作らせて C++ で
  その存在を見る**しかない。
