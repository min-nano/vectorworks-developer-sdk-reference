...
| [Parametric Objects](Parametric%20Objects.md) | PIO 全般。パスの型・ストーリバウンド・パラメータ名（**表は種別ごとに不変＝解決は 1 度でよい**・呼び出しのコスト）・設定ダイアログ抑止・スタイル・ポップアップの値・構造材の自動結合（作る API は無い）・自作 PIO の作法 |
