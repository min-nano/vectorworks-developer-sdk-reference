...
| [Parametric Objects](Parametric%20Objects.md) | PIO 全般。パスの型・ストーリバウンド・パラメータ名・設定ダイアログ抑止・スタイル・ポップアップの値・**部材長／スパンのパラメータは無い**（実体はパスで測る）・構造材の自動結合（作る API は無い）・自作 PIO の作法・**リセットをまとめられるか**（まとめても速くならない／`doRegen=false` ＋ 0 長で半分） |
