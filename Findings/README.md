...
| [Parametric Objects](Parametric%20Objects.md) | PIO 全般。パスの型・ストーリバウンド・パラメータ名（**表は種別ごとに不変＝解決は 1 度でよい**・呼び出しのコスト）・設定ダイアログ抑止・**スタイル**（SDK だけで作れる／`CreatePluginStyle` は呼んではいけない／**スタイルはストーリバウンドも配る**——書いたバウンドが作り直しで置き換わる。順序では避けられず、逃げ道は by-instance）・ポップアップの値・構造材の自動結合（作る API は無い）・**構造材のスパン／部材長はパラメータで読めない**（長さはパスの両端で測る）・自作 PIO の作法・**リセットをまとめられるか**（まとめても速くならない／`doRegen=false` ＋ **Z の差 0** のパスで半分——鉛直・水平・斜めのどれでも） |
