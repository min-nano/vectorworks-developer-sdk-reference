...
| `DoMenuText` / `DoMenuTextByName`（メニューコマンドを名前で起動） | **無い**（直接は） | 汎用の「メニュー項目を実行する」API が ISDK / VWFC に無い（`Include/vs.py` にスクリプト用の宣言があるだけ）。**間接経路**（`IVectorScriptEngine` / `IPythonScriptEngine` の `ExecuteScript` でスクリプトごと実行させる）は存在し、コマンド実行中に呼べる。VectorScript 経由なら `DoMenuTextByName('Undo', 0)` は**到達して取り消しが掛かり、連続で呼べば複数段戻る**（実測で 3 段）が、掛かるのは**スクリプト自身がした操作**まで——**呼び出し元（C++）が undo イベントの外でしたことは取り消しスタックに載っておらず、戻らない**（実機確認済み）。しかも**取り消しを実行すると、呼び出し側が開いたままの undo イベントが終わらされる**（`ExecuteScript` そのものや失敗の後始末ではなく、取り消しの実行が犯人。実機で切り分け済み）。Python 経由は **`ExecuteScript` から `vs.*` を呼ぶと VectorWorks ごと落ちる**——**`ScriptContext_Begin` → `ScriptContext_Run` を使えば通り、挙動は VectorScript 版と同一**（実機確認済み）。[Undo](Undo.md) |
