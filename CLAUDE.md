...
4. **Findings へ反映する PR を作る。** 確認水準の印（無印＝実機確認済み・【推定】・
   ...
   **印を付けたからといってマージしてよいわけではない**——その調査が確かめると決めた項目に
   未確認が残っているなら、答えが来るまで PR は open のまま待つ（下記「PR とマージ」3）。
...
2. **待つのは「リファレンスに何を書くか」だけ。** 実機でしか確かめられない挙動を
   無印（＝実機確認済み）で `Findings/` に書く PR は、ユーザーが VectorWorks 実機で確認し
   「確認できた」と伝えるまで open のまま待つ。未確認の記述が確認済みの顔をして蓄積すると、
   リファレンス全体の信頼が崩れる——**待つ理由はこれひとつ**で、以下の判断はすべてここから
   導かれる。
3. **その調査が確かめると決めた項目に未確認が残っているなら、マージしない。**
   **【未確認】の印を付けることは、マージしてよい理由にならない。** 印は「書いたものが
   確認済みの顔をしない」ためのもので、**調べ残しを畳んで先へ進むための道具ではない**。
   わざわざ実機まで持っていった調査で、**取りに行けば取れる答え**を未確認のまま確定させると、
   その issue は二度と開き直されない——**未確認がそのまま永久に残る**。
   - **目視でしか分からないこと**（保存確認ダイアログが出たか・図面がどう見えるか・
     落ちたか）は**プローブのログに写らない**。**ユーザーに尋ね、返事が来るまで PR は
     open のまま待つ。黙ってマージしない。**
   - 尋ねる前に、**機械で確かめ直せないかを考える**。「ダイアログが出たか」は目視だが、
     「文書が閉じたか」は `GetOpenFilesList` の件数で分かる。**目視を頼む前にプローブへ
     1 行足す**（そのほうが次に走らせる人の手間も減る）。
   - **例外は、最初からその issue の範囲外だと決めていたもの**だけ。範囲外なら
     **別の issue に切り出してから**マージする（`Findings/` の「未確認のまま残っているもの」
     に置き去りにしない）。**範囲内で答えを取り損ねたものは例外にしない。**
   - `Closes #<issue>` を書いた PR をマージすると issue が閉じる。**issue を閉じてよいのは
     その調査の問いが片付いたときだけ**なので、この規約は「未確認を残したまま issue を
     閉じない」と同じことを言っている。
   - **既にマージしてしまったら**、issue を開き直し、残っている項目を並べたコメントを
     足す（`Findings/` の記述はそのままでよい——印が付いているなら嘘は書いていない）。
4. **実機確認の要らない変更は CI green で自動マージしてよい。** 【推定】【ヘッダ根拠】の
   印付きの追記、`sdk-grep` の結果の転記、目次・体裁・スクリプト・CI 設定など。
   **ただし 3 が先に効く**——「実機で確かめると決めて実際に走らせた」調査の反映は、
   印が付いていてもここには入らない。判断に迷うなら 2・3 に倒す。
5. **道具の作り替えは待たない——`plugin/` を触る PR も CI green でマージしてよい。**
   プラグインは**調べるための道具**であって、リファレンスそのものではない。CI が
   確かめられるのは「コンパイルとリンクが通る」ことまでで、ダイアログが出るか・プローブが
   走るかは実機でしか分からないが、**壊れていれば次に使ったときすぐ分かる**（そして直して
   進めればよい）——リファレンスの誤りと違って、黙って積もることがない。作り方・使い方の
...
- **結果は PR コメント（PR が無ければ、プローブ先頭の `[issue #N]` が指す issue への
  ...
  - `result=failed` は**プローブが `probe.fail(...)` を呼んだか例外で止まった**という
    意味で、投稿そのものの失敗ではない。
  - `recovered=yes` が付いていたら、**そのプローブは VectorWorks ごと落として**いる
    （結果ダイアログが出る前に終了した）。載っているのは落ちるまでに書けた行で、
    **末尾がそのまま最後に通った場所**である。落ち方そのものが知見なので、これを
    「投稿の不具合」と読み違えない。
  - 頼んでよいのは「（新しいビルドに入れ替えて）もう一度走らせてください」だけにする。
    ログを貼る・番号を打つ・設定を開く——このどれかを頼んでいるなら仕組みが壊れている
    合図なので、頼む前に直す。
  - **`plugin/src/**` を直す PR は、公開ビルドでは確かめられない**（公開ビルドは main の
    殻で作られ、PR からは `probes/runtime/` しか取り込まないため）。そのときは PR の
    Actions の成果物（`VwSdkProbes-mac` / `-windows`）を手で入れてもらう——そのビルドは
    出所に PR 番号を持たないので、**動いているビルドのブランチから宛先を引く**
    （`plugin/README.md`「PR のビルドを手で入れて確かめるとき」）。
