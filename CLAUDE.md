...
2. **既に答えが無いか確かめる。** `Findings/` の該当トピックと「打ち切った調査」を先に
   ...
   版——が変わらない限り）。
3. **作業ブランチで調査する。** SDK を読めば答えが出る問いは `ci-debug` の `sdk-grep` /
   `sdk-ls`（**ヘッダだけでなく `SDKLib/Source` の VWFC 実装 `.cpp` も見る**——「なぜ
   この関数が失敗するか」はたいていそこに書いてある）、「この呼び出しはコンパイルが通るか」は `probes/` にスニペットを置いて
   `compile`（下記「CI デバッグ」）。
